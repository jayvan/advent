...
public class Advent15_02 {
  public static void Main() {
    ...
    for (int power = 4; power <= 200; power++) {
      world = new World(width, height, walls, units, power);

      while (!world.GameOver) {
        world.Update();
      }

      if (world.elfDeaths == 0) {
        break;
      }
    }
...
public class World {
  ...
  public World(int width, int height, HashSet<Tuple<int, int>> walls, List<Unit> units, int power) {
    this.grid = new bool[width, height];
    this.units = new List<Unit>(units.Count);
    foreach (Unit unit in units) {
      this.units.Add(new Unit(unit.IsElf, unit.X, unit.Y, power));
    }
  ...
  public void Update() {
    ...
    this.elfDeaths += this.units.Count(unit => unit.IsElf && unit.Health <= 0);
  ...
  public void PrintStats() {
    ...
    Console.WriteLine($"Power level: {this.units.First().Power}");
    Console.WriteLine($"Elf Deaths: {this.elfDeaths}");
    Console.WriteLine($"Battle finished after {Steps} rounds");
    Console.WriteLine($"Remaining HP: {totalHp}");
    Console.WriteLine($"Answer: {totalHp * Steps}");
  }
}
...
public class Unit : IComparable {
  ...
  public Unit(bool elf, int x, int y, int power = 3) {
    this.IsElf = elf;
    this.X = x;
    this.Y = y;
    this.Health = 200;
    this.Power = elf ? power : 3;
  }
  ...
  private void AttackNeighbour(bool[,] grid, List<Unit> units) {
    ...
    toHit?.Hit(grid, Power);
  }

  private void Hit(bool[,] grid, int damage) {
    this.Health -= damage;
    if (this.Health <= 0) {
